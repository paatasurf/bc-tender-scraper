...
def collect_events(session: Any, scope: str) -> tuple[list[dict[str, Any]], int]:
    ...
    permit_query = select(Permit).where(
        Permit.company_id.isnot(None), Permit.is_active.is_(True)
    )
    ...
    for row in session.scalars(permit_query).all():
        event_date = parse_event_date(row.issue_date) or parse_event_date(
        ...
        events.append(
            {
                "kind": "permit",
                "id": row.id,
                "raw_company_id": int(row.company_id),
                "event_date": event_date,
                "city": row.city,
                "value": parse_value(row.project_value),
                "confidence": row.canonical_merge_confidence,
            }
        )
        ...
        events.append(
            {
                ...
                "value": float(row.award_value or 0),
...
def canonical_map(
    session: Any, raw_ids: set[int]
) -> tuple[dict[int, int], dict[int, Company]]:
    companies = (
        {
            row.id: row
    ...
    alias_map = {
        int(row.alias_company_id): int(row.canonical_company_id) for row in aliases
    }
...
def fastest_growing(
    session: Any, scope: str, lookback_days: int, limit: int, minimum_activity: int
) -> dict[str, Any]:
    today = date.today()
    ...
    for cid, windows in groups.items():
        ...
        if company is None:
            continue
        current_value = sum(e["value"] for e in current)
        previous_value = sum(e["value"] for e in previous)
        ...
        candidates.append(
            {
                ...
                "previous_activity_count": len(previous),
                "absolute_delta": delta,
                "relative_growth": round(relative, 4),
                "permits_current": sum(e["kind"] == "permit" for e in current),
                "permits_previous": sum(e["kind"] == "permit" for e in previous),
                "awards_current": sum(e["kind"] == "award" for e in current),
                "awards_previous": sum(e["kind"] == "award" for e in previous),
                "known_value_current": round(current_value, 2),
                "known_value_previous": round(previous_value, 2),
                "latest_activity_date": latest.isoformat(),
                "evidence_confidence": (
                    "HIGH"
                    if all(e["confidence"] is not None for e in current)
                    else "MEDIUM"
                ),
                "evidence": [
                ...
                "_recency": recency,
                "_value_delta": max(0.0, current_value - previous_value),
            }
        )
    ...
    for row in candidates:
        ...
        economy = 10 * (
            math.log1p(row.pop("_value_delta")) / math.log1p(max_value)
            if max_value > 0
            else 0
        )
    ...
    candidates.sort(
        key=lambda row: (
            -row["growth_score"],
            -row["absolute_delta"],
            -row["current_activity_count"],
            row["company_id"],
        )
    )
    return {
        "analysis": "Fastest-growing Recent Company Activity",
        "geography": scope,
        ...
        "invalid_event_dates_excluded": invalid,
        "current_window": {
        ...
        "scoring": {
            "absolute_activity_increase": 45,
            "relative_increase_capped": 25,
            "recency": 20,
            "economic_significance": 10,
            "deterministic": True,
        },
        "data_gaps": [
            "Tenders and early signals excluded because reliable company attribution is unavailable."
        ],
...
def geographic_footprint(
    session: Any, company_id: int, lookback_days: int, limit: int
) -> dict[str, Any]:
    events, invalid = collect_events(session, "BC")
    mapping, companies = canonical_map(
        session, {e["raw_company_id"] for e in events} | {company_id}
    )
    canonical_id = mapping.get(company_id, company_id)
    selected = [
        e
        for e in events
        if mapping.get(e["raw_company_id"]) == canonical_id
        and e["event_date"] >= date.today() - timedelta(days=lookback_days)
    ]
    ...
    for geography, rows in grouped.items():
        data.append(
            {
                "geography": geography,
                "total_activity": len(rows),
                "permits": sum(e["kind"] == "permit" for e in rows),
                "awards": sum(e["kind"] == "award" for e in rows),
                "known_value": round(sum(e["value"] for e in rows), 2),
                "latest_activity": max(e["event_date"] for e in rows).isoformat(),
                "share_of_company_activity": (
                    round(len(rows) / total, 4) if total else 0
                ),
                "evidence_confidence": (
                    "HIGH"
                    if all(e["confidence"] is not None for e in rows)
                    else "MEDIUM"
                ),
                "evidence": ["permits.city", "contract_awards.winner_city"],
            }
        )
    data.sort(
        key=lambda row: (-row["total_activity"], -row["known_value"], row["geography"])
    )
    return {
        "analysis": "Company Geographic Footprint",
        "company_id": canonical_id,
        "company_name": (
            (company.display_name or company.name).strip() if company else None
        ),
        "lookback_days": lookback_days,
        "raw_company_id": company_id,
        "canonical_company_id": canonical_id,
        "activity_records_considered": total,
        "invalid_event_dates_excluded": invalid,
        "data_gaps": [
            "Tenders and early signals excluded because reliable company attribution is unavailable."
        ],
