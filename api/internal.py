...
def _resolve_import_run_id(run_id: str | None) -> str:
    """Validate import ordering and bind omitted run ids to the active scrape run."""
    try:
...
def import_csvs(
    ...
    sync: Annotated[
        bool,
        Query(description="When true, run import synchronously and return tracked counts."),
    ] = False,
) -> dict[str, Any]:
    _require_manual_pipeline()
