#!/usr/bin/env python3
"""Surrey identity-aware import canary planner (Class A -- no database
writes).

Fetches only PermitNumber and ApplicantOrganization from Surrey's
official ArcGIS layer, then models the full three-tier decision tree of
db.surrey_permit_import.upsert_surrey_permit_identity_aware (update via
official_source_id / update via not-yet-bridged legacy prefix / insert)
inside one explicit REPEATABLE READ, READ ONLY database transaction.
There is no --apply or --allow-production mode -- this script never
writes to permits, never runs the import, never touches Company or
company_id.
"""
...
DEFAULT_ARTIFACT_PATH = ROOT / "exports" / "surrey_identity_import_canary.json"
...
def fetch_official_source_rows() -> list[dict[str, Any]]:
    """Fetch the full current ArcGIS window. Row shape matches
    scraper/surrey_permits.py's convention: "external_id" holds the
    official PermitNumber, "applicant" holds the raw ApplicantOrganization."""
    http = create_session()
    try:
        ...
        while len(rows) < expected_count:
            ...
            for feature in features:
                ...
                rows.append(
                    {
                        "external_id": attrs.get("PermitNumber"),
                        "applicant": attrs.get("ApplicantOrganization"),
                    }
                )
...
def run_canary(
    ...
    *,
    ...
    try:
        ...
        report = plan_surrey_identity_import(session, rows=source_rows)
    finally:
