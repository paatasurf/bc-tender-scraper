#!/usr/bin/env python3
"""Apply the full reviewed Surrey official-source-identity bridge batch
(Class C).

Requires a fresh schema artifact from the Class-A dry-run
(scripts/run_permit_official_source_id_bridge_dryrun.py) at the exact
...
-- a single atomic transaction, rolled back on any exception, digest
mismatch, or partial result.

Changes only a still-blank Permit.official_source_id. Never touches
applicant, company_id, address, external_id, project_value, or any
lifecycle/scoring field, and never creates a Company.
"""
...
from pipeline.permit_official_source_id_bridge import (  # noqa: E402
    ARTIFACT_SCHEMA_VERSION,
    apply_permit_official_source_id_bridge,
)
from scripts.run_permit_official_source_id_bridge_dryrun import (  # noqa: E402
    TRANSACTION_MODE,
...
DEFAULT_ARTIFACT_PATH = (
    ROOT / "exports" / "permit_official_source_id_bridge_dryrun_production_class_a.json"
)
...
class PermitOfficialSourceIdBridgeFullError(ValueError):
    """Raised when the reviewed artifact or full-apply result is unsafe."""
...
def load_and_validate_artifact(
    ...
    *,
    ...
    try:
        ...
        raise PermitOfficialSourceIdBridgeFullError(
            "bridge artifact is missing, unreadable, or invalid JSON"
        ) from exc
    if not isinstance(payload, dict):
        raise PermitOfficialSourceIdBridgeFullError(
            "bridge artifact must be a JSON object"
        )
    if payload.get("artifact_schema_version") != ARTIFACT_SCHEMA_VERSION:
        raise PermitOfficialSourceIdBridgeFullError(
            "bridge artifact schema version mismatch"
        )
    if payload.get("git_commit_sha") != current_git_sha:
        raise PermitOfficialSourceIdBridgeFullError("bridge artifact git SHA mismatch")
    if payload.get("source") != "surrey":
        raise PermitOfficialSourceIdBridgeFullError("bridge artifact source mismatch")
    if payload.get("transaction_mode") != TRANSACTION_MODE:
        raise PermitOfficialSourceIdBridgeFullError(
            "bridge artifact was not built read-only"
        )

    counts = payload.get("counts")
    if not isinstance(counts, dict):
        raise PermitOfficialSourceIdBridgeFullError(
            "bridge artifact counts are missing"
        )

    candidate_count = payload.get("candidate_count")
    integer_fields = {
        "candidate_count": candidate_count,
        "recoverable_official_source_id": counts.get("recoverable_official_source_id"),
        **{name: counts.get(name) for name in SAFETY_COUNTER_FIELDS},
    }
    for name, value in integer_fields.items():
        if type(value) is not int or value < 0:
            raise PermitOfficialSourceIdBridgeFullError(
                f"bridge artifact has invalid integer field: {name}"
            )

    if counts["recoverable_official_source_id"] != candidate_count:
        raise PermitOfficialSourceIdBridgeFullError(
            "bridge artifact candidate count invariant failed"
        )
    for field in SAFETY_COUNTER_FIELDS:
        if counts[field] != 0:
            raise PermitOfficialSourceIdBridgeFullError(
                f"bridge artifact contains unsafe source/key condition: {field}"
            )

    if candidate_count <= 0:
        raise PermitOfficialSourceIdBridgeFullError(
            "bridge artifact is stale: candidate_count is 0 -- nothing left to "
            "bridge, or the artifact predates a prior successful apply run"
        )

    if not _DIGEST_RE.fullmatch(str(payload.get("candidate_set_digest") or "")):
        raise PermitOfficialSourceIdBridgeFullError(
            "bridge artifact has invalid digest field: candidate_set_digest"
        )
    return payload


def execute_full_bridge(session, *, source_rows, artifact) -> dict[str, Any]:
...
def main() -> int:
    ...
    guard_destructive_db_from_args(
        args,
        script_name=_SCRIPT,
        operation="Surrey official-source-identity bridge full apply",
