#!/usr/bin/env python3
"""Surrey official-source-identity bridge dry-run (Class A -- no database
writes).

Fetches only PermitNumber from Surrey's official ArcGIS layer, then builds
an aggregate-only plan inside one explicit REPEATABLE READ, READ ONLY
database transaction: which existing Surrey Permit rows could safely have
their still-blank official_source_id populated, by joining the exact
16-character legacy prefix of the official PermitNumber to the existing
Permit.external_id. Exact match only -- no fuzzy matching, no writes.
There is no --apply or --allow-production mode.
"""
...
DEFAULT_ARTIFACT_PATH = (
    ROOT / "exports" / "permit_official_source_id_bridge_dryrun.json"
)
TRANSACTION_MODE = "REPEATABLE READ, READ ONLY"
SOURCE_FIELDS = "PermitNumber"
    ...
    try:
        ...
        while len(rows) < expected_count:
            ...
            for feature in features:
                ...
                rows.append({"PermitNumber": attrs.get("PermitNumber")})
