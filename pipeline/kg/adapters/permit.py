...
from pipeline.kg.constants import ADAPTER_VERSION_PERMIT_V1, ENTITY_TYPE_PERMIT, SCHEMA_VERSION_V1
...
def _permit_fingerprint_parts(row: dict[str, str], *, source: str) -> tuple[str, str, str, str]:
    return (
        source,
...
def derive_permit_external_id(row: dict[str, str], *, source: str) -> str:
    ...
    if not fingerprint[1] or not fingerprint[2]:
        raise ValueError("permit row lacks address and project_value for fingerprint external_id")

    digest = hashlib.sha256(canonical_json(list(fingerprint)).encode("utf-8")).hexdigest()[:32]
    return f"fp:{digest}"
...
def build_permit_payload(row: dict[str, str], *, source: str) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        field: row.get(field, "") if field not in {"company_id", "canonical_merge_confidence"} else row.get(field)
...
class PermitObservationAdapter(ObservationAdapter):
    ...
    def source_key(self, raw: Any, **context: Any) -> str:
        source = context.get("source") or (raw.get("source") if isinstance(raw, dict) else "")
...
def dual_write_permit_observations(session, rows: list[dict[str, str]], *, source: str) -> None:
