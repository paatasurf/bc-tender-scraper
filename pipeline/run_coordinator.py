...
def _is_incomplete_tender_scrape(state: RunState | None) -> bool:
    if state is None or state.tender_scrape_finished_at:
        return False
    return any(step not in state.completed_tender_scrapes for step in TENDER_SCRAPE_STEPS)


def begin_or_resume_tender_scrape_run(run_id: str | None = None) -> RunState:
    """Start or reuse the active standalone tender scrape run."""
    with _LOCK:
        state = _load_state()
        if run_id is None and _is_incomplete_tender_scrape(state):
            assert state is not None
            target = state
        elif state is not None and run_id is not None and state.run_id == run_id:
            target = state
        else:
            target = RunState(run_id=run_id or str(uuid.uuid4()), phase="tender_scrape")

        now = _iso(_utc_now())
        target.phase = "tender_scrape"
        target.tender_scrape_started_at = target.tender_scrape_started_at or now
        target.scrape_phase_started_at = target.scrape_phase_started_at or now
        _save_state(target)
        return target
