...
def _has_incomplete_tender_scrape(state: RunState) -> bool:
    if state.finished_at or state.tender_scrape_finished_at:
        return False
...
def begin_or_resume_tender_scrape_run(run_id: str | None = None) -> RunState:
    """Start a tender scrape run, or reuse the active one for implicit requests."""
    with _LOCK:
        state = _load_state()
        actual_run_id = run_id
        if actual_run_id is None and state is not None and _has_incomplete_tender_scrape(state):
            actual_run_id = state.run_id
        actual_run_id = actual_run_id or str(uuid.uuid4())
