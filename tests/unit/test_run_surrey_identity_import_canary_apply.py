"""Safety-contract tests for the PR-EN1F-3 fixed Class-C Surrey import
canary apply runner (scripts/run_surrey_identity_import_canary_apply.py)."""
...
import scripts.run_surrey_identity_import_canary_apply as runner
...
def test_missing_or_malformed_artifact_is_rejected(tmp_path):
    with pytest.raises(runner.SurreyImportCanaryApplyError):
...
def test_unsafe_risk_counter_is_rejected(tmp_path, field):
    ...
    with pytest.raises(runner.SurreyImportCanaryApplyError, match=field):
        runner.load_and_validate_artifact(
            _write(tmp_path / "artifact.json", payload),
            current_git_sha="a" * 40,
        )
...
def test_insufficient_planned_updates_is_rejected(tmp_path):
    payload = _artifact()
    payload["counts"]["planned_updates"] = 19
    with pytest.raises(runner.SurreyImportCanaryApplyError, match="planned updates"):
        runner.load_and_validate_artifact(
            _write(tmp_path / "artifact.json", payload),
            current_git_sha="a" * 40,
        )
...
def test_insufficient_planned_inserts_is_rejected(tmp_path):
    payload = _artifact()
    payload["counts"]["planned_inserts"] = 4
    with pytest.raises(runner.SurreyImportCanaryApplyError, match="planned inserts"):
        runner.load_and_validate_artifact(
            _write(tmp_path / "artifact.json", payload),
            current_git_sha="a" * 40,
        )
...
# --- execute_import_canary: commit/rollback contract ----------------------
...
def test_execute_import_canary_commits_exactly_20_and_5(monkeypatch):
    session = _Session()
    captured = {}
    ...
    def fake_apply(_session, **kwargs):
        ...
        return {
            "eligible_updates": 925,
            "eligible_inserts": 269,
            "selected_updates": 20,
            "selected_inserts": 5,
            "updated": 20,
            "inserted": 5,
    ...
    monkeypatch.setattr(runner, "apply_surrey_identity_import_canary", fake_apply)
    result = runner.execute_import_canary(
...
@pytest.mark.parametrize(
    "result",
    [
        {"updated": 19, "inserted": 5},
        {"updated": 20, "inserted": 4},
        {"updated": 21, "inserted": 5},
        {"updated": 20, "inserted": 6},
        {"updated": 0, "inserted": 0},
    ],
)
def test_execute_import_canary_rolls_back_non_exact_result(monkeypatch, result):
    session = _Session()
    monkeypatch.setattr(
        runner,
    ...
    with pytest.raises(runner.SurreyImportCanaryApplyError):
        runner.execute_import_canary(session, source_rows=[], artifact=_artifact())
    assert session.commits == 0
    assert session.rollbacks == 1


def test_execute_import_canary_rolls_back_writer_exception(monkeypatch):
    session = _Session()
    monkeypatch.setattr(
        runner,
        "apply_surrey_identity_import_canary",
...
def test_execute_import_canary_rolls_back_on_stale_digest_from_live_refetch(
    monkeypatch,
):
    session = _Session()
...
def test_script_has_no_configurable_limit_or_force_flag():
    ...
    row = next(
        ...
        if "run_surrey_identity_import_canary_apply.py" in line
    )
...
def test_main_reaches_class_c_guard_before_network_or_database(monkeypatch, tmp_path):
    ...
    assert "import canary" in captured["operation"]
