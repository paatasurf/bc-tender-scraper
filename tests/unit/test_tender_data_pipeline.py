...
def test_begin_or_resume_tender_scrape_run_reuses_active_run(
    coordinator_state: Path,
) -> None:
    first_run_id = coordinator.begin_or_resume_tender_scrape_run()
    coordinator.mark_tender_scrape_step(first_run_id, "scrape-federal")

    second_run_id = coordinator.begin_or_resume_tender_scrape_run()
...
def test_begin_or_resume_tender_scrape_run_rolls_after_import_complete(
    coordinator_state: Path,
) -> None:
    first_run_id = coordinator.begin_or_resume_tender_scrape_run()
    for step in coordinator.TENDER_SCRAPE_STEPS:
        coordinator.mark_tender_scrape_step(first_run_id, step)
    coordinator.begin_import(first_run_id)
    coordinator.complete_import(first_run_id)
    ...
    state = coordinator.get_run_state()
    assert state is not None
    assert state.run_id == second_run_id
...
def test_internal_tender_scrapes_without_body_share_active_run(
    ...
    enqueued_run_ids = [call.args[3] for call in enqueue_step.call_args_list]
    assert len(enqueued_run_ids) == 3
    assert len(set(enqueued_run_ids)) == 1
