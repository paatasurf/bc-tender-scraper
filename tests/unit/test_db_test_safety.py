...
def test_require_local_test_database_skips_when_unset():
    with patch("tests.db_test_safety._ci_skips_db_integration", return_value=False):
...
def test_require_local_test_database_skips_on_ci_without_ci_database_url():
    with patch.dict(os.environ, {"CI": "true", "CI_DATABASE_URL": ""}, clear=False):
        with pytest.raises(pytest.skip.Exception, match="skipped on CI"):
            require_local_test_database()
