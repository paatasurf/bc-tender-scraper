"""Runner contract tests for the Surrey official-source-identity bridge
dry-run."""
...
import scripts.run_permit_official_source_id_bridge_dryrun as runner
...
def test_source_fetch_paginates_and_requests_only_permit_number(monkeypatch):
    http = _Http()
    calls = []
    responses = iter(
        [
                    ...
                    "features": [
                        {
                            "attributes": {
                                ...
                                "ApplicantOrganization": "must not escape",
            ...
            _Response(
                {
                    "features": [
                        {"attributes": {"PermitNumber": "26-222222-001-00/ABC"}}
                    ],
    ...
    assert len(rows) == 2
    assert set(rows[0]) == {"PermitNumber"}
    assert "ApplicantOrganization" not in rows[0]
...
def test_run_uses_one_read_only_transaction_and_rolls_back(monkeypatch, tmp_path):
    ...
    def session_factory(*, bind):
        session = _BridgeSession(bind=bind)
...
def test_failure_rolls_back_and_writes_no_artifact(monkeypatch, tmp_path):
    engine = _Engine()
    monkeypatch.setattr(runner, "Session", _BridgeSession)
    monkeypatch.setattr(
        runner,
        "plan_permit_official_source_id_bridge",
...
def test_classification_lists_runner_as_class_a():
    ...
    row = next(
        ...
        if "run_permit_official_source_id_bridge_dryrun.py" in line
    )
    assert "| A |" in row
