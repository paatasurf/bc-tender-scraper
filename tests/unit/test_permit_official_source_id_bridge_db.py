"""Local-Postgres proof for the Surrey official-source-identity bridge
writer -- changes only Permit.official_source_id and never commits."""
...
def test_real_update_changes_only_official_source_id_and_never_commits(db_session):
    unique = uuid.uuid4().int % 1_000_000
    legacy_id = f"28-{unique:06d}-001-00"
    ...
    result = apply_permit_official_source_id_bridge(
        ...
        candidate_limit=1,
        expected_candidate_set_digest=compute_bridge_digest([(permit_id, source_id)]),
    )
    ...
    row = (
        ...
        .execute(
            text(
                ...
                "project_value, architect, issue_date, description, contractor, "
                "source, city, external_id, company_id, canonical_merge_confidence, "
    ...
    assert row.applicant == "Original Applicant"
    ...
    assert row.issue_date == "2026-07-21"
