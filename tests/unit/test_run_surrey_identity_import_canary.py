"""Runner contract tests for the Surrey identity-aware import canary."""
...
import scripts.run_surrey_identity_import_canary as runner
...
def test_source_fetch_paginates_and_maps_to_scraper_row_shape(monkeypatch):
    http = _Http()
    calls = []
    responses = iter(
        [
    ...
    assert len(rows) == 2
    assert set(rows[0]) == {"external_id", "applicant"}
    assert rows[0]["external_id"] == "26-111111-001-00/ABC"
    assert "ProjectAddress" not in rows[0]
...
def test_run_uses_one_read_only_transaction_and_rolls_back(monkeypatch, tmp_path):
    ...
    def session_factory(*, bind):
        session = _CanarySession(bind=bind)
    ...
    artifact = runner.run_canary(engine, source_rows=[], artifact_path=path)
...
def test_failure_rolls_back_and_writes_no_artifact(monkeypatch, tmp_path):
    engine = _Engine()
    monkeypatch.setattr(runner, "Session", _CanarySession)
    monkeypatch.setattr(
        runner,
        "plan_surrey_identity_import",
    ...
    with pytest.raises(RuntimeError, match="secret"):
        runner.run_canary(engine, source_rows=[], artifact_path=path)
...
def test_classification_lists_runner_as_class_a():
    ...
    row = next(
        ...
        if "run_surrey_identity_import_canary.py" in line
    )
    assert "| A |" in row
